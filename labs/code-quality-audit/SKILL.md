...
description: 实验中的可量化代码质量审计，仅供开发与评估，不建议日常安装使用。合并重构坏味道与软件反模式视角（31 项检查项，去重后统一分类），通过"确定性分派 + 多子代理普查 + 结构摘要 map-reduce"对整个仓库做可审计覆盖的深度审查，输出机器可读的 findings.json、覆盖回执、维度分数向量与加权总分。
---

# 代码质量审计（可量化）

> **Labs 状态：** 此 Skill 仍在研究和验证中，仅供开发与评估，不建议日常安装使用。接口、行为和输出格式可能随时变化。
